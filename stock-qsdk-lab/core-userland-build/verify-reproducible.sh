...
final_output=${CORE_USERLAND_FINAL_OUTPUT:-"$build_dir/candidate-out/core-userland-2026"}
repro_tmp=$(mktemp -d "${TMPDIR:-/tmp}/sbe-core-userland-repro.XXXXXX")
round1=$repro_tmp/round1
round2=$repro_tmp/round2

cleanup() {
	case "$repro_tmp" in
		/tmp/sbe-core-userland-repro.*|/var/tmp/sbe-core-userland-repro.*)
			rm -rf "$repro_tmp"
			;;
	esac
}
...
CORE_USERLAND_OUTPUT=$round1 "$build_dir/build-candidates.sh" "$source_root"
CORE_USERLAND_OUTPUT=$round2 "$build_dir/build-candidates.sh" "$source_root"
...
[ "$(wc -l < "$list1" | tr -d ' ')" -eq 9 ] || {
	printf 'ERROR: reproducibility gate expected exactly nine IPKs\n' >&2
	exit 1
}
...
{
	printf 'core-userland-2026 forced-clean reproducibility: PASS\n'
	printf 'SOURCE_DATE_EPOCH=1789005952\n'
...
printf 'PASS: two forced-clean builds produced byte-identical IPKs and payloads.\n'
