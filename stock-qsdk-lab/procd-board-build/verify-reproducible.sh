...
final_output=${PROCD_BOARD_FINAL_OUTPUT:-"$build_dir/candidate-out/procd-board"}
repro_tmp=$(mktemp -d "${TMPDIR:-/tmp}/sbe-procd-repro.XXXXXX")
round1=$repro_tmp/round1
round2=$repro_tmp/round2

cleanup() {
	case "$repro_tmp" in
		/tmp/sbe-procd-repro.*|/var/tmp/sbe-procd-repro.*) rm -rf "$repro_tmp" ;;
	esac
}
trap cleanup EXIT HUP INT TERM

PROCD_BOARD_OUTPUT=$round1 "$build_dir/build-candidate.sh" "$source_root"
PROCD_BOARD_OUTPUT=$round2 "$build_dir/build-candidate.sh" "$source_root"

find "$round1" -maxdepth 1 -type f -name '*.ipk' -exec basename '{}' ';' | LC_ALL=C sort > "$repro_tmp/list1"
find "$round2" -maxdepth 1 -type f -name '*.ipk' -exec basename '{}' ';' | LC_ALL=C sort > "$repro_tmp/list2"
...
[ "$(wc -l < "$repro_tmp/list1" | tr -d ' ')" -eq 1 ] || {
	printf 'ERROR: reproducibility gate expected exactly one IPK\n' >&2
	exit 1
}
grep -Eq '^sbe-procd2020-board-candidate_.*\.ipk$' "$repro_tmp/list1" || {
...
{
	printf 'QSDK procd board-info forced-clean reproducibility: PASS\n'
...
find "$final_output" -maxdepth 1 -type f -name 'sbe-procd2020-board-candidate_*.ipk' -delete
...
printf 'PASS: two forced-clean builds produced a byte-identical procd candidate IPK and payload.\n'
