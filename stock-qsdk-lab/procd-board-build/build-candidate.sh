...
jobs=${QSDK_JOBS:-4}
output=${PROCD_BOARD_OUTPUT:-"$build_dir/candidate-out/procd-board"}
...
[ "$(uname -s)" = Linux ] || fail 'procd candidate must be built inside Linux'
...
archive_source=$build_dir/distfiles/$PROCD_ARCHIVE
archive_target=$qsdk_dir/dl/$PROCD_ARCHIVE
if [ -f "$archive_target" ]; then
	[ "$(sha256_file "$archive_target")" = "$PROCD_ARCHIVE_SHA256" ] ||
		fail "QSDK distfile exists with the wrong hash: $archive_target"
else
	cp "$archive_source" "$archive_target"
fi

package_name=sbe-procd2020-board-candidate
...
config_backup=$(mktemp "${TMPDIR:-/tmp}/sbe-procd-config.XXXXXX")
...
CONFIG_PACKAGE_sbe-procd2020-board-candidate=m
EOF
make -C "$qsdk_dir" defconfig

minimal_config=$(mktemp "${TMPDIR:-/tmp}/sbe-procd-minimal.XXXXXX")
...
find "$qsdk_dir/bin" -type f -name 'sbe-procd2020-board-candidate_*.ipk' -delete
find "$output" -maxdepth 1 -type f -name 'sbe-procd2020-board-candidate_*.ipk' -delete
make -C "$qsdk_dir" "package/$package_name/clean"
make -C "$qsdk_dir" -j"$jobs" NO_DEPS=1 "package/$package_name/compile" V=s

artifact=$(find "$qsdk_dir/bin" -type f -name 'sbe-procd2020-board-candidate_*.ipk' -print |
	LC_ALL=C sort | tail -n 1)
[ -n "$artifact" ] || fail 'build completed without the procd candidate IPK'
cp "$artifact" "$output/"
...
(cd "$output" && sha256sum "./$(basename "$artifact")") > "$output/PACKAGE_SHA256SUMS"
printf 'BUILT, NOT LIVE-INSTALLED: procd board-info candidate is in %s\n' "$output"
printf 'No rootfs, image, kernel or device-write target was invoked.\n'
