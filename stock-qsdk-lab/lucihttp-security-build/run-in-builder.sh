...
image=${LUCIHTTP_BUILDER_IMAGE:-openwrt-builder:ubuntu-22.04-arm64}
...
docker run --rm --network none --platform linux/arm64 \
	...
	-e GIT_CONFIG_VALUE_1=/repo/stock-qsdk-lab/lucihttp-security-build/sources/lucihttp \
	-v "$repo_root:/repo:rw" \
	-v "$source_root:/workspace/qsdk-spf12.2-locked:rw" \
	"$image" /bin/bash -lc \
	'mkdir -p "$HOME" && /repo/stock-qsdk-lab/lucihttp-security-build/verify-reproducible.sh /workspace/qsdk-spf12.2-locked'
