...
image=${CGI_IO_BUILDER_IMAGE:-openwrt-builder:ubuntu-22.04-arm64}
mode=${1:-all}
...
case "$mode" in
	repro)
		command='/repo/stock-qsdk-lab/cgi-io-security-build/verify-reproducible.sh /workspace/qsdk-spf12.2-locked'
		qsdk_mode=rw
		;;
	audit)
		command='/repo/stock-qsdk-lab/cgi-io-security-build/audit-artifact.sh'
		qsdk_mode=ro
		;;
	all)
		command='/repo/stock-qsdk-lab/cgi-io-security-build/verify-reproducible.sh /workspace/qsdk-spf12.2-locked && /repo/stock-qsdk-lab/cgi-io-security-build/audit-artifact.sh'
		qsdk_mode=rw
		;;
	*)
...
docker run --rm --pull=never --network none --platform linux/arm64 \
	...
	-e GIT_CONFIG_VALUE_1=/repo/stock-qsdk-lab/cgi-io-security-build/sources/cgi-io \
	-v "$repo_root:/repo:ro" \
	-v "$build_dir:/repo/stock-qsdk-lab/cgi-io-security-build:rw" \
...
[ ! -L "$source_root/qsdk/package/sbe-qsdk-lab/sbe-cgi-io2026-security-candidate" ] || {
	printf 'ERROR: cgi-io package overlay symlink remains after Docker exited\n' >&2
	exit 1
}
exit "$status"
