...
final_output=${RRDNS_FINAL_OUTPUT:-"$build_dir/candidate-out/rrdns-modern"}
repro_tmp=$(mktemp -d "${TMPDIR:-/tmp}/sbe-rrdns-repro.XXXXXX")
...
RRDNS_MODERN_OUTPUT=$round1 "$build_dir/build-candidate.sh" "$source_root"
RRDNS_MODERN_OUTPUT=$round2 "$build_dir/build-candidate.sh" "$source_root"
...
grep -Eq '^sbe-rpcd-mod-rrdns20170710-modern-candidate_.*\.ipk$' "$list1" || {
...
{
	printf 'rpcd-mod-rrdns current-official candidate-only forced-clean reproducibility: PASS\n'
...
printf 'PASS: two candidate-only forced-clean rrdns builds produced a byte-identical IPK and payload.\n'
